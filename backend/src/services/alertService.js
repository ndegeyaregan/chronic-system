...
const checkVitalAlerts = async (memberId, vitals) => {
  ...
  // Batch-load all relevant thresholds in one query
  const metrics = checks.map(c => c.metric);
  const threshRes = await pool.query(
    `SELECT DISTINCT ON (metric) * FROM vital_thresholds
     WHERE metric = ANY($1::text[])
       AND (condition_id = ANY($2::uuid[]) OR condition_id IS NULL)
     ORDER BY metric, condition_id NULLS LAST`,
    [metrics, conditionIds]
  );

  const thresholdMap = {};
  for (const row of threshRes.rows) {
    thresholdMap[row.metric] = row;
  }

  for (const check of checks) {
    const thresh = thresholdMap[check.metric];
    if (!thresh) continue;
    const outOfRange =
      (thresh.min_value !== null && check.value < thresh.min_value) ||
      (thresh.max_value !== null && check.value > thresh.max_value);
...
// Check for lab tests due within 2 days and send daily reminders
// ── DAILY push: morning (09:00) and evening (18:00) ONLY 2 days before due date ──
  ...
  const res = await pool.query(
    `SELECT lt.*, m.first_name, m.fcm_token
     ...
     WHERE lt.status = 'pending'
       ...
       AND (CURRENT_DATE + INTERVAL '2 days') >= DATE(lt.due_date AT TIME ZONE 'Africa/Johannesburg')`
  );
...
// ── END-OF-MONTH EMAIL: send reminder on last day of month for all pending lab tests ──
const sendLabTestEndOfMonthEmail = async () => {
  const res = await pool.query(
    `SELECT lt.*, m.first_name, m.email
     FROM lab_tests lt
     JOIN members m ON m.id = lt.member_id
     WHERE lt.status = 'pending'
       AND (lt.last_end_of_month_email_at IS NULL
            OR lt.last_end_of_month_email_at < NOW() - INTERVAL '27 days')
       AND m.email IS NOT NULL`
  );
  ...
  for (const row of res.rows) {
    ...
    const isOverdue = new Date(row.due_date) < new Date();
    await notificationService.sendToMember(row.member_id, {
      type: 'lab_test_end_of_month_email',
      title: isOverdue ? '⚠️ Lab Test Overdue' : '🔬 End of Month Lab Test Reminder',
      message: isOverdue
        ? `Hi ${row.first_name}, your ${testLabel} test (due ${dueStr}) is overdue. Please visit your nearest lab as soon as possible to complete this important test.`
        : `Hi ${row.first_name}, as we approach the end of the month, please remember that your ${testLabel} test is due on ${dueStr}. Please schedule your appointment with your nearest lab.`,
      channel: ['email'],
      email: row.email,
      firstName: row.first_name,
    });
    await pool.query(
      'UPDATE lab_tests SET last_end_of_month_email_at = NOW(), updated_at = NOW() WHERE id = $1',
      [row.id]
    );
  }
};

// ── SMS ON DUE DATE: send SMS reminder on the actual due date ──
const sendLabTestDueDateSms = async () => {
  ...
  const res = await pool.query(
    `SELECT lt.*, m.first_name, m.phone
     ...
     JOIN members m ON m.id = lt.member_id
     WHERE lt.status = 'pending'
       AND DATE(lt.due_date AT TIME ZONE 'Africa/Johannesburg') = CURRENT_DATE
       AND (lt.last_due_date_sms_at IS NULL
            OR DATE(lt.last_due_date_sms_at AT TIME ZONE 'Africa/Johannesburg') < CURRENT_DATE)
       AND m.phone IS NOT NULL`
  );
  ...
  for (const row of res.rows) {
    const testLabel = row.test_type.replace(/_/g, ' ');
    await notificationService.sendToMember(row.member_id, {
      type: 'lab_test_due_date_sms',
      title: 'Lab Test Due Today',
      message: `Sanlam Health: Your ${testLabel} test is due TODAY! Please visit your nearest lab to complete this important test.`,
      channel: ['sms'],
      phone: row.phone,
      firstName: row.first_name,
    });
    await pool.query(
      'UPDATE lab_tests SET last_due_date_sms_at = NOW(), updated_at = NOW() WHERE id = $1',
      [row.id]
    );
  }
};
...
module.exports = {
  ...
  sendLabTestEndOfMonthEmail,
  sendLabTestDueDateSms,
