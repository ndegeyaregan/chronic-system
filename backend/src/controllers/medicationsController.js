...
const adminUpdateAssignment = async (req, res) => {
  try {
    const { id } = req.params;
    const { dosage, frequency, notes, reminder_enabled, next_refill_date, refill_interval_days, pharmacy_id, edit_note } = req.body;
    const adminName = [req.user.first_name || req.user.name?.split(' ')?.[0], req.user.last_name || req.user.name?.split(' ')?.slice(1)?.join(' ')].filter(Boolean).join(' ') || req.user.email;
    ...
    const result = await pool.query(
      `UPDATE member_medications
       SET dosage = COALESCE($1, dosage),
           ...
           refill_interval_days = COALESCE($6, refill_interval_days),
           pharmacy_id = COALESCE($7, pharmacy_id),
           edited_by = $8,
           edited_by_name = $9,
           edit_note = $10,
           updated_at = NOW()
       ...
       RETURNING *`,
      [
        dosage || null, frequency || null, notes || null,
        reminder_enabled != null ? reminder_enabled : null,
        next_refill_date || null,
        refill_interval_days ? parseInt(refill_interval_days) : null,
        pharmacy_id || null,
        req.user.id, adminName, edit_note || null, id,
      ]
    );
    if (!result.rows.length) return res.status(404).json({ message: 'Assignment not found' });
    ...
    try {
      await pool.query(
        ...
        [req.user.id, 'update', 'member_medication', id, JSON.stringify({ dosage, frequency, notes, edit_note, edited_by_name: adminName })]
      );
    } catch (_) { /* audit log is best-effort */ }

    return res.json(result.rows[0]);
  } catch (err) {
    console.error('adminUpdateAssignment error:', err);
    return res.status(500).json({ message: 'Server error' });
  }
};
