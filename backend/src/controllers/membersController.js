  ...
  try {
    ...
    const result = await pool.query(
      `SELECT
       ...
       FROM members m ${joinClause} ${whereClause}
       GROUP BY m.id
...
const registerMember = async (req, res) => {
  try {
    const {
      ...
      plan, plan_type, conditions, date_of_birth, id_number,
    } = req.body;
    if (!member_number || !first_name || !last_name || !date_of_birth) {
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO members
          (member_number, first_name, last_name, email, phone, plan_type, date_of_birth, id_number, is_active)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,TRUE)
         RETURNING id, member_number, first_name, last_name, email, phone, plan_type, date_of_birth, id_number, is_active, created_at`,
        [
          member_number,
          first_name,
          last_name,
