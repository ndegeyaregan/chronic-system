...
const sendCampaign = async (req, res) => {
  try {
    ...
    // Get target members
    let memberQuery = 'SELECT id FROM members WHERE is_active = TRUE';
    const params = [];
    ...
    // Insert notification records for each member × each channel
        ...
        insertPromises.push(
          pool.query(
            `INSERT INTO notifications (member_id, type, channel, title, message, status, sent_at)
             VALUES ($1, 'campaign', $2, $3, $4, 'sent', NOW())`,
            [m.id, ch, title, message]
          )
        );
      }
    }
