...
const createAdmin = async (req, res) => {
  try {
    ...
    // Only eddyregan4@gmail.com can create super_admin role
    if (role === 'super_admin' && req.user.email !== 'eddyregan4@gmail.com') {
...
const updateAdmin = async (req, res) => {
  try {
    ...
    // Only eddyregan4@gmail.com can change someone to super_admin
    if (role === 'super_admin' && req.user.email !== 'eddyregan4@gmail.com') {
...
const getContentAdminPerformance = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT 
        id, email, name, first_name, last_name,
        COALESCE((
          SELECT COUNT(DISTINCT id) FROM treatment_plans 
          LIMIT 1000
        ), 0) AS total_treatment_plans,
        COALESCE((
          SELECT COUNT(DISTINCT id) FROM members 
          LIMIT 1000
        ), 0) AS members_managed,
        COALESCE((
          SELECT COUNT(DISTINCT id) FROM appointments 
          LIMIT 1000
        ), 0) AS total_appointments
      FROM admins
      WHERE role = 'content_admin' AND is_active = TRUE
      ORDER BY created_at DESC`
    );
    return res.json(result.rows);
  } catch (err) {
    console.error('getContentAdminPerformance error:', err);
    return res.status(500).json({ message: 'Server error' });
  }
};

module.exports = { listAdmins, createAdmin, updateAdmin, toggleAdminStatus, resetAdminPassword, getContentAdminPerformance };
