...
class ChronicDashboardScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ChronicDashboardScreen> createState() => _DashboardScreenState();
}

class _DashboardScreenState extends ConsumerState<ChronicDashboardScreen> {
  String _wellnessTip  = '';
  ...
  Future<void> _loadWeatherTip() async {
    try {
      ...
      // wttr.in sometimes returns HTML (rate-limit, errors) — fall back
      if (raw.contains('<') || raw.contains('<!DOCTYPE') || raw.isEmpty) {
        _applyTimeBasedTip(); return;
      }
              ...
              actions: [
                ...
                PopupMenuButton<String>(
                    ...
                    const PopupMenuItem(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 10),
                          Text('Logout', style: TextStyle(color: Color(0xFFDC2626), fontWeight: FontWeight.w600)),
                        ],
                      ),
                    ),
                  ],
                  child: Container(
                    ...
                    child: Center(
                      child: Text(
                        _initials(member?.firstName, member?.fullName),
...
// ── Health Score Card ─────────────────────────────────────────────────────────
// TODO: The "Export Health Report" button in profile_screen.dart should only
// be visible for chronic members. Currently it is always shown. A future phase
// should wrap that button with a Consumer that checks isChronicMemberProvider.
