...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  Future<void> _saveMedicationAndNext() async {
    ...
    try {
      await dio.post('/medications/member', data: {
        'name': name,
        'dosage': _medDosageCtrl.text.trim().isNotEmpty
            ? _medDosageCtrl.text.trim()
            : 'As prescribed',
        'frequency': _medFreqCtrl.text.trim().isNotEmpty
            ? _medFreqCtrl.text.trim()
            : 'Daily',
      });
    } on DioException catch (_) {
      // Medication save is best-effort; proceed regardless
    } finally {
      if (mounted) setState(() => _savingMedication = false);
    }
    _nextPage();
  }
          ...
          PageView(
            ...
            children: [
              ...
              _MedicationPage(
                ...
                dosageCtrl: _medDosageCtrl,
                freqCtrl: _medFreqCtrl,
                onNext: _saveMedicationAndNext,
                onSkip: _nextPage,
                saving: _savingMedication,
              ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text(
                'Quickly add your primary medication — you can add more later.',
                style: TextStyle(color: Colors.white70, fontSize: 15, height: 1.4),
              ),
              ...
              const Spacer(),
              _OnboardingButton(
                label: saving ? null : 'Add & Continue',
                onTap: saving ? null : onNext,
                white: true,
                loading: saving,
              ),
