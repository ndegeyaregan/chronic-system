...
class ConditionEducationScreen extends ConsumerWidget {
  ...
  List<ConditionInfo> _matchedConditions(List<String> memberConditions) {
    final results = <ConditionInfo>[];
    for (final cond in memberConditions) {
      final lower = cond.toLowerCase();
      if (lower.contains('hypertension') ||
          lower.contains('high blood pressure')) {
        if (!results.any((c) => c.title.contains('Hypertension'))) {
          results.add(_conditionContent['hypertension']!);
        }
      }
      if (lower.contains('diabetes') || lower.contains('diabetic')) {
        if (!results.any((c) => c.title.contains('Diabetes'))) {
          results.add(_conditionContent['diabetes']!);
        }
      }
      if (lower.contains('heart') ||
          lower.contains('coronary') ||
          lower.contains('cardiac')) {
        if (!results.any((c) => c.title.contains('Heart'))) {
          results.add(_conditionContent['heart']!);
        }
      }
      if (lower.contains('copd') ||
          lower.contains('asthma') ||
          lower.contains('lung')) {
        if (!results.any((c) => c.title.contains('COPD'))) {
          results.add(_conditionContent['copd']!);
        }
      }
      if (lower.contains('kidney') || lower.contains('renal') || lower.contains('ckd')) {
        if (!results.any((c) => c.title.contains('Kidney'))) {
          results.add(_conditionContent['kidney']!);
        }
      }
    }
    return results;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final conditions = memberState.member?.conditions ?? [];
    final matched = _matchedConditions(conditions);
      ...
      body: ListView(
        ...
        children: [
          ...
          if (matched.isEmpty)
            ...
            ...matched.map((info) => _ConditionCard(info: info)),
