...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry && !isLoginRequest) {
      ...
      const refreshToken = localStorage.getItem('sanlam_admin_refresh_token');
      if (!refreshToken) {
      ...
      try {
        ...
        localStorage.setItem('sanlam_admin_token', data.token);
        localStorage.setItem('sanlam_admin_refresh_token', data.refreshToken);
