...
import { getMedicationOverview, getMedicationCatalogue, createMedication, assignMedicationToMember, updateMedication, stopAssignment, updateAssignmentRefill, getAssignmentDoseLogs } from '../api/medications';
...
export default function MedicationsPage() {
  ...
  const assignmentColumns = [
    ...
    },
    {
      key: 'has_media',
      header: 'Media',
      render: (value) => <Badge status={value ? 'published' : 'inactive'} label={value ? 'Attached' : 'None'} />,
    },
    {
      key: 'id',
      header: 'Actions',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {mediaAssignment.prescription_file_url && (
              <div>
                ...
                <a href={`http://localhost:5000${mediaAssignment.prescription_file_url}`} target="_blank" rel="noreferrer"
            ...
            {mediaAssignment.photo_url && (
              <div>
                ...
                <img src={`http://localhost:5000${mediaAssignment.photo_url}`} alt="Medication" style={{ maxWidth: '100%', borderRadius: '8px', border: '1px solid #e2e8f0' }} />
              </div>
            )}
            {mediaAssignment.audio_url && (
              <div>
                <p style={{ margin: '0 0 6px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Audio Instructions</p>
                <audio controls src={`http://localhost:5000${mediaAssignment.audio_url}`} style={{ width: '100%' }} />
              </div>
            )}
            {mediaAssignment.video_url && (
              <div>
                <p style={{ margin: '0 0 6px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Video Instructions</p>
                <video controls src={`http://localhost:5000${mediaAssignment.video_url}`} style={{ maxWidth: '100%', borderRadius: '8px' }} />
              </div>
            )}
          </div>
        </Modal>
