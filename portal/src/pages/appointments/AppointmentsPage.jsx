...
import { getAllAppointments, updateAppointmentStatus, createAppointmentForMember } from '../../api/appointments';
...
export default function AppointmentsPage() {
  ...
  const { data: membersData } = useQuery({
    queryKey: ['members-create'],
    queryFn: () => getMembers({ limit: 1000 }).then((r) => r.data),
    retry: false,
  ...
  const { data: hospitalsData } = useQuery({
    queryKey: ['hospitals-create'],
    queryFn: () => getHospitals({ limit: 1000 }).then((r) => r.data),
    retry: false,
    placeholderData: [],
  });
  ...
  // Auto-populate condition when member is selected
  useEffect(() => {
    if (selectedMember?.conditions && selectedMember.conditions.length > 0) {
      const conditionsList = Array.isArray(selectedMember.conditions) 
        ? selectedMember.conditions.join(', ')
        : typeof selectedMember.conditions === 'string'
        ? selectedMember.conditions
        : '';
      setValue('condition', conditionsList);
    }
  }, [selectedMemberId, selectedMember, setValue]);
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Condition',
      render: (value, row) => {
        // Use appointment condition if available, otherwise try to get member conditions
        if (value) return value;
        const member = membersData?.members?.find((m) => m.id === row.member_id);
        if (member?.conditions) {
          const conditionsList = Array.isArray(member.conditions)
            ? member.conditions.join(', ')
            : typeof member.conditions === 'string'
            ? member.conditions
            : '';
    ...
    },
    {
      ...
      render: (value, row) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <span style={{ ...sourceStyle(value), padding: '3px 10px', borderRadius: '999px', fontSize: '12px', fontWeight: '600', whiteSpace: 'nowrap', display: 'inline-block', width: 'fit-content' }}>
            {value}
          </span>
          {value === 'Admin booked' && row.admin_name && (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <Button variant="success" onClick={() => { reset(); setCreateModal(true); }}>+ Create Appointment</Button>
      </div>
      ...
      {createModal && (
        <Modal title="Create New Appointment" onClose={() => { setCreateModal(false); reset(); }} width="600px">
          <form onSubmit={handleSubmit(handleCreateAppointment)} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <p style={{ margin: '0 0 8px', fontSize: '14px', color: '#64748b' }}>
              Create a new appointment for a member by selecting the member, hospital, and appointment date.
            </p>

            <div style={{ position: 'relative' }}>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', fontWeight: '500', color: 'var(--text)' }}>Member *</label>
              <input
                type="hidden"
              ...
              >
                <input
                  type="text"
                  placeholder="Search member by name, number, or email…"
                  value={showMemberDropdown ? memberSearch : selectedMemberName}
                  onChange={(e) => {
                    ...
                    filteredMembers.map((member) => (
                      <div
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          background: selectedMemberId === member.id ? '#f0f4f8' : '#fff',
                          ':hover': { background: '#f8fafc' },
                        }}
                        onMouseEnter={(e) => (e.currentTarget.style.background = '#f8fafc')}
                        onMouseLeave={(e) => (e.currentTarget.style.background = selectedMemberId === member.id ? '#f0f4f8' : '#fff')}
                      >
                        <p style={{ margin: 0, fontSize: '14px', fontWeight: '500' }}>
                          {member.first_name} {member.last_name}
                        </p>
                        <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                          {member.member_number || 'No #'} • {member.email || 'No email'}
                        </p>
                      </div>
                    ))
                  ) : (
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', fontWeight: '500', color: 'var(--text)' }}>Hospital/Provider *</label>
              <select
              ...
              >
                ...
                {(Array.isArray(hospitalsData) ? hospitalsData : []).map((hospital) => (
            ...
            <Input label="Appointment Date *" name="appointment_date" type="date" register={register} required />
            <Input label="Preferred Time" name="preferred_time" type="time" register={register} />
            <Input label="Condition/Reason" name="condition" register={register} placeholder="e.g., Diabetes checkup" />
            <Input label="Notes" name="reason" register={register} placeholder="Additional notes" />
